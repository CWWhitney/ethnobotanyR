# Quantitative Ethnobotany “quantitative
# ethnobotany,” coined by Prance et al.
# (1987)
...
# The formatR package, by Yihui Xie, makes it easier to clean up poorly formatted code.
...
# Example data has 3 plants, 10
# interviewees, 10 categories data should
# be rows of species names and columns of
# use categories populated with counts of
# uses per person (should be 0 or 1
# values) (max value =N; total number of
# people interviewed)

# Sum of all UR for all species===== test
sum(ethnobotanydata[, -c(1:2)])
# function
use_sums <- function(data) {
    sum(data[, -c(1:2)])
}

use_sums(ethnobotanydata)

# Use Reports (UR) per Species=====
# example
URstestdata <- ethnobotanydata
URstestdata$URps <- rowSums(URstestdata[,
    -c(1:2)])
library(plyr)
plyr::ddply(URstestdata, ~sp_name, summarise, URs = sum(URps))
# alternative example
# aggregate(URstestdata$URps,
# by=list(URs=URstestdata$sp_name),
# FUN=sum)

# function
use_reports <- function(data) {

    data$URps <- rowSums(data[, -c(1:2)])
    library(plyr)
    data_URs <- plyr::ddply(data, ~sp_name, summarise,
        URs = sum(URps))

    print(data_URs)
}

use_reports(ethnobotanydata)

# Cultural Importance index (CI)=====

cultural_importance <- function(data) {
    data$URps <- rowSums(data[, -c(1:2)])
    library(plyr)
    data_URs <- plyr::ddply(data, ~sp_name, summarise,
        URs = sum(URps))
...
cultural_importance(ethnobotanydata)  #maximum CI is N

# use value (UV) index per species=====
# sum of number of different uses
# mentioned by each informant / n)
# (simplified by Rossato et al. 1999 and
# Albuquerque et al. 2006)

# test
UVstestdata <- ethnobotanydata
UVstestdata$UVps <- rowSums((UVstestdata[,
    -c(1:2)]) > 0)
plyr::ddply(UVstestdata, ~sp_name, summarise, UVs = sum(UVps)/(length(unique(UVstestdata$informant))))

# function
use_value <- function(data) {
    data$UVps <- rowSums((data[, -c(1:2)]) >
        0)
    plyr::ddply(data, ~sp_name, summarise, UVs = sum(UVps)/(length(unique(informant))))
}

use_value(ethnobotanydata)

# number of uses (NU)====

# test
NUdata <- ethnobotanydata
NUdataaggr <- aggregate(NUdata[, -c(1:2)],
    by = list(sp_name = NUdata$sp_name),
    FUN = sum)
...
number_uses(ethnobotanydata)

# Frequency of Citation (FC) per
# species===== test
FCstestdata <- ethnobotanydata
FCstestdata$FCps <- rowSums((FCstestdata[,
    -c(1:2)]) > 0)
FCstestdata$FCps[FCstestdata$FCps > 0] <- 1
plyr::ddply(FCstestdata, ~sp_name, summarise, FCs = sum(FCps))

# function
frequency_citation <- function(data) {
    data$FCps <- rowSums((data[, -c(1:2)]) >
        0)
    data$FCps[data$FCps > 0] <- 1
    plyr::ddply(data, ~sp_name, summarise, FCs = sum(FCps))
}

frequency_citation(ethnobotanydata)

# Relative Frequency of Citation (RFC)
# Pardo-de-Santayana (2003)=====

rel_frequency_citation <- function(data) {
    data$FCps <- rowSums((data[, -c(1:2)]) >
        0)
    data$FCps[data$FCps > 0] <- 1
    plyr::ddply(data, ~sp_name, summarise, RFCs = sum(FCps)/(length(unique(informant))))
}

rel_frequency_citation(ethnobotanydata)


# Relative Importance Index (RI)
# Pardo-de-Santayana (2003)===== test
RFCstestdata <- ethnobotanydata
RFCstestdata$FCps <- rowSums((RFCstestdata[,
    -c(1:2)]) > 0)
RFCstestdata$FCps[RFCstestdata$FCps > 0] <- 1
RFCstestdata2 <- plyr::ddply(RFCstestdata, ~sp_name,
    summarise, FCs = sum(FCps))
...
# function
relative_importance <- function(data) {
    RFCstestdata <- data
    RFCstestdata$FCps <- rowSums((RFCstestdata[,
        -c(1:2)]) > 0)
...
relative_importance(ethnobotanydata)

# Cultural Value Index (CV) Reyes-García
# et al. (2006)====


CVNUdata <- ethnobotanydata
CVNUdataaggr <- aggregate(CVNUdata[, -c(1:2)],
    by = list(sp_name = CVNUdata$sp_name),
    FUN = sum)
CVNUdataaggr[, -1][CVNUdataaggr[, -1] > 0] <- 1
CVNUdataaggr$NUs <- rowSums(CVNUdataaggr[,
    -1])
CVNUdataaggr$NUdivNC <- CVNUdataaggr$NUs/ncol(CVNUdata[,
    -c(1:2)])
NUdivNC <- CVNUdataaggr[, c(1, length(names(CVNUdataaggr)))]

CVFCsdata <- ethnobotanydata
CVFCsdata$FCps <- rowSums((CVFCsdata[, -c(1:2)]) >
    0)
CVFCsdata$FCps[CVFCsdata$FCps > 0] <- 1
CVFCdata2 <- plyr::ddply(CVFCsdata, ~sp_name, summarise,
    FCs = sum(FCps))
...
library(plyr)
CVURdata2 <- plyr::ddply(CVURdata, ~sp_name, summarise,
    URs = sum(URps))
...
cultural_value <- function(data) {
    ...
    CVURdata$URps <- rowSums(CVURdata[, -c(1:2)])
    library(plyr)
    CVURdata2 <- plyr::ddply(CVURdata, ~sp_name,
...
cultural_value(ethnobotanydata)
