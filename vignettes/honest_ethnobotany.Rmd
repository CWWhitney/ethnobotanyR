...
### Calculating All Indices and Confidence Intervals Easily
> **Interpreting the confidence interval:**
> The CI shows the range where the true proportion of informants citing each species likely falls, given your sample. If CIs overlap between species, differences are not statistically reliable. Wide intervals mean less certainty; overlapping intervals mean no clear difference between species.

The `index_with_ci()` function in ethnobotanyR now calculates citation proportions and confidence intervals directly from your ethnobotany data. This approach is robust and transparent, matching the workflow of other package functions.

**New Use Case:**

You can use `index_with_ci()` to calculate the proportion of informants citing each species, along with a 95% confidence interval, using only your data frame. No need to manually count citations or call index functions first.

**Example:**

```r{index_with_ci}
# Use built-in ethnobotany data
index_with_ci(ethnobotanydata)
```

This returns a data frame with each species, the citation proportion, and the confidence interval.

You can also use custom data:

```r{index_with_ci_test}
eb_data <- data.frame(
  informant = c('User_1', 'User_2', 'User_3', 'User_4'),
  sp_name = c('sp_1', 'sp_1', 'sp_2', 'sp_2'),
  Use_A = c(1, 0, 1, 1),
  Use_B = c(0, 1, 0, 1)
)
...
This makes it easy to report both the citation proportion and its confidence interval for each species. For more complex indices, custom handling may be needed, but most standard proportion-based metrics are covered.
