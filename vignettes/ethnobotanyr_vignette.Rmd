...
`URs_plot()` shows these results in a radial plot using the `ggplot2` library. 

```{r}
ethnobotanyR::URs_plot(ethnobotanydata)
```
...
The `FLs()` function calsulates the fidelity level (FL) per species in the study. It is a way of calculting the percentage of informants who use a plant for the same purpose as compared to all uses of all plants.
