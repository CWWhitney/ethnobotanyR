...
## A Critical Perspective on Ethnobotany Indices
...
While widely used in the literature, traditional ethnobotany indices have significant limitations that researchers should consider:

- **Oversimplification**: Complex cultural relationships reduced to single numbers
- **Context stripping**: Remove cultural, ecological, and historical context
- **Methodological issues**: Sensitive to sampling design and interview methods
- **Theoretical concerns**: Often lack strong theoretical foundations

**When to use these indices:**
- Preliminary exploratory analysis
- Comparison with existing literature that uses similar methods
- As input variables for more sophisticated models (not as final results)

**When to avoid these indices:**
- As primary evidence for policy decisions
- For cross-cultural comparisons without careful validation
- As standalone measures of "importance"
...
Figures can help to visually represent the main ideas or key results of ethnobotany work. Here we walk through a few of the functions within the `ethnobotanyR` package for generating figures [@R-ethnobotanyR]. 
...
The use report `URs()` is the most basic ethnobotany calculation. The function calculates the reported uses for each species in the data set. 
...
- Raw counts don't account for sampling effort or cultural significance
- Can be misleading if informant expertise varies significantly
- Best used as input for more sophisticated analyses rather than final results
...
In order to use these techniques effectively, it is important for researchers to have access to programming tools that allow them to run the analyses. The `ethnobotanyR` package offers a few handy tools to make these work with standard quantitative ethnobotany data. By having access to these tools, researchers can more easily and efficiently run these analyses and incorporate the results into their research projects.
