...
The `FLs()` function calsulates the fidelity level (FL) per species in teh study. It is a way of calculting the percentage of informants who use a plant for the same purpose as compared to all uses of all plants.

\begin{equation}
FLs = \frac {Np}{Nc} × 100 
\end{equation}

where $Np$ is the number of informants that use a particular plant for a specific purpose, and $Nc$ is the total number of citations, a.k.a. the sum of all Use Reports, which can be calculated with `URsum()`.
