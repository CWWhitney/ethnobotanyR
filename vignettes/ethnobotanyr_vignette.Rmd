...
The inputs to the function are informant responses to the use category for each plant, an estimate of informant's confidence_in_answers with the plant, and the number of possible answers. This can be calculated with `URsum` or given as a value.

Depending on the size of the data this function can return a rather large set of probabilities. Our example data `ethnobotanydata` returns a data frame with `r nrow(ethnobotanyR::ethno_bayes_consensus(ethnobotanydata, confidence_in_answers=0.5, prior = -1))` rows and `r ncol(ethnobotanyR::ethno_bayes_consensus(ethnobotanydata, confidence_in_answers=0.5, prior = -1))` columns.
...
We can use this to create an uninformative prior about the answers. This gives a 50% chance for either 'use' or 'no use'.

```{r}
#set the number of answers to '2' i.e. 'use' or 'no use'.
  answers <- 2
...
Here we use `dplyr` `recode` to reset the informant name factor variable as numeric [@dplyr]. This way we can set a prior for the informants skill for the `confidence_in_answers` input. Assuming that informants have a varying degree of skill that we can assign as a prior for the likelihood that the data we have are correct for `sp_a`.
...
ethno_sp_a_bayes <- ethno_bayes_consensus(ethno_sp_a, answers = 2,
                    confidence_in_answers = ethno_compet_sp_a, 
                    prior = ethno_sp_a_prior)
```

Create a simple heatmap of the results. The `heatmap` function in R [@R-base] provides a good initial assessment of the results and can be a nice first look at the probability matrix that comes out of the `ethno_bayes_consensus` function. It includes the `hclust` hierarchical cluster analysis for relationships among both the answers and the uses. This may be useful for looking for similarities among a number of uses or possible answers when there are more than just 'use' and 'non use' (see below). Here the '1' and '2' represent 'use' and 'no use'. The `hclust` for these is not very informative since there are only 2. However, the `hclust` for the various uses might be helpful in thinking about how the strength of the information about different use categories for `sp_a` are grouped together. 
```{r heatmap}
...
```{r}
 ethno_sp_a_rich <- data.frame(replicate(4,sample(0:10,20,rep=TRUE)))
 names(ethno_sp_a_rich) <- gsub(x = names(ethno_sp_a_rich), 
                                pattern = "X", replacement = "Use_")  
 ethno_sp_a_rich$informant <- sample(c('User_1', 'User_2'), 
                                20, replace=TRUE)
 ethno_sp_a_rich$sp_name <- sample(c('sp_1', 'sp_2'), 
                                20, replace=TRUE)
```

Define the `confidence_in_answers` of the data from these new informants in the simulated ethnobotany data. With `User_1` we have high confidence because perhaps we gather this information through 'walk in the woods' or another method we feel good about. With `User_2` we assign less confidence. Maybe did our work in a rush or gathered second hand or in another way that gives us less confidence. 
```{r}
...
```{r fig.width=7, fig.height=7}
ethno_sp_a_bayes <- ethno_bayes_consensus(ethno_sp_a_rich,
                        answers = 10, 
                        confidence_in_answers = ethno_compet_sp_a_rich, 
